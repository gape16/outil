    ...
    <div class="control-block" >
      ...
      <div class="control-icon more has-items" id="first">
        ...
        if($result['notif_C']==0){
          ...
          $query_notif_codec=$bdd->prepare("SELECT id_client, categorie, etat_achat.etat, photo FROM achat_photos left join user on achat_photos.id_graph = user.id_user left join etat_achat on achat_photos.id_etat_achat = etat_achat.id_etat_achat WHERE id_achat > ? and achat_photos.id_etat_achat = 1 order by id_achat DESC");
        ...
        $dd=$bdd->prepare("SELECT num_client, raison_social, lien_CMS, photo, IDGPP, client.id_etat, etat.nom_etat FROM client left join user on client.envoi_maquette=user.id_user left join etat on client.id_etat = etat.id_etat where (client.id_etat = 2 or client.id_etat= 5)");
        ...
        <div class="more-dropdown more-with-triangle triangle-top-center">
          <div class="ui-block-title ui-block-title-small">
            ...
            <a href="explore_admin.php">Allez aux partage de codes</a>
          </div>
            ...
            <ul class="notification-list friend-requests notif_list">
             ...
             <?php }else{
              ...
              <li>
                ...
                <div class="notification-event">
                  <a href="explore_admin.php" class="h6 notification-friend"><?php echo $value['titre'];?></a>
        ...
        <a href="explore_admin.php" class="view-all bg-blue">Voir tous les codes</a>
      </div>
    </div>
...
<div class="control-icon more has-items" id="ss">
  <svg class="olymp-happy-faces-icon left-menu-icon" data-toggle="tooltip" data-placement="right" data-original-title="" title=""><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="icons/icons.svg#olymp-share-icon"></use></svg>

  <div class="label-avatar bg-primary label_retours" <?php if($dd->rowCount() == 0){ echo "style='display:none;'"; }?>><?php if($dd->rowCount()!=0){ echo $dd->rowCount();}?></div>

  <div class="more-dropdown more-with-triangle triangle-top-center">
    <div class="ui-block-title ui-block-title-small">
      <h6 class="title">Retours de contrôle</h6>
      <a href="accueil.php">Allez aux retours</a>
    </div>

    <div class="mCustomScrollbar" data-mcs-theme="dark">
      <ul class="notification-list friend-requests veille_retours">
       <?php 
       if($result['notif_D']==0){?>
       <li>
        Aucune notification
      ...
      <?php }else{
        ...
        <li>
          ...
          <div class="notification-event">
            <a href="accueil.php" class="h6 notification-friend"><?php echo $value['raison_social'];?></a>
            <span class="chat-message-item"><?php echo utf8_encode($value['nom_etat']);?></span>
          </div>
          <span class="notification-icon">
            <?php echo $value['num_client'];?>
          </span>
        </li>
        <?php } 
      }?>
  ...
  <a href="accueil.php" class="view-all bg-primary">Voir les retours</a>
</div>
</div>

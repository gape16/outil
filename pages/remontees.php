...
if (isset($_COOKIE['id_statut'])) {
	...
	<head>
		...
		p.categorie_remontees {
			margin: 0;
			line-height: 38px;
			padding-left: 10px;
			border-left: 2px solid #9a9fbf;
		}
		.wrapper_content p {
...
<body>
	...
	<div class="container">
		<div class="row">
			<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12">
				...
				<div class="ui-block responsive-flex1200">
					<div class="ui-block-title">
						<div class="w-select">
							<select class="filters-remontees">
								<option value="*" data-filter="*"><a href="#" selected>Toutes les états</option>
									<?php foreach ($selection_etat_remontees as $key => $value) {?>
									<option class="btn-state_<?php echo($value['id_categorie_remontees']) ?>" value=".etat_<?php echo($value['id_etat_remontees']) ?>"><?php echo utf8_encode($value['etat_remontees']) ?></option>
									<?php } ?>
								</select>

								<select class="filters-remontees-2">
									<option value="*" data-filter="*"><a href="#" selected>Toutes les catégories</option>
										...
										<option class="btn-cate_<?php echo($value['id_categorie_remontees']) ?>" value=".cat_<?php echo($value['id_categorie_remontees']) ?>"><?php echo utf8_encode($value['categorie_remontees']) ?></option>
										<?php } ?>
									</select>


									<div class="form-group with-button is-empty">
										<input class="form-control search" type="text" placeholder="Rechercher dans les remontées..">
										<div class="hax-btn">
									...
									<a href="#" class="btn btn-primary btn-md-2 trigger">Ajouter une remontée</a>
								</div>
							</div>
							<!-- fin -->
						</div>
			...
			<div class="container">
		<!-- <div class="toggle_remontees">
			<svg class="olymp-plus-icon add_remontees" style="fill: #fff">
				<use xlink:href="../icons/icons.svg#olymp-plus-icon""></use>
			</svg>
		</div>
		<p class="h4 tac">Faire une remontée</p> -->
		<div class="row form">
			<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<div class="ui-block">
					<div class="ui-block-title">
	...
	<div class="container">
		<div class="row sorting-container grid" data-layout="masonry">
			<?php foreach ($query_select_remontees as $key => $value) {
				...
				?>
				<div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-xs-12 grid-item element-item sorting-item cat_<?php echo($value['id_categorie_remontees']) ?>  etat_<?php echo($value['id_etat_remontees']) ?>">
					<!-- Post -->
						...
						<div class="post__author author vcard inline-items">
							<div class="wrapper">
								...
								<div class="wrapper_content" contenteditable="false">
									...
									<pre class="break"><p><?php echo htmlentities(utf8_encode($value['description']));?></p></pre>
								</div>
...
$('.filters-remontees').on( 'change', function() {
  // get filter value from option value
  var filterValue = $(this).val();
  var filterValue2 = $('.filters-remontees-2').val();
  $grid.isotope({ filter: filterValue  + filterValue2});
  console.log(filterValue);
});

$('.filters-remontees-2').on( 'change', function() {
  // get filter value from option value
  var filterValue = $(this).val();
  var filterValue2 = $('.filters-remontees').val();
  $grid.isotope({ filter: filterValue  + filterValue2});
  console.log(filterValue);
});
...
$('[class*="btn-state_"]').on('click', function(e){
	...
	e.stopPropagation();
	var btn_cate = "";
	$('[class*="btn-cate_"]').each(function(){
		if ($(this).hasClass('this_btn')) {
			btn_cate = $(this).attr('data-filter');
		}
	})
	var check="btn-state_";
	var cls = $(this).attr('class').split(' ');
	console.log(cls);
	$('[class*="btn-state_"]').removeClass('this_btn');
	...
	$grid.isotope({ filter: filterValue + btn_cate });
})

$('[class*="btn-cate_"]').on('click', function(e){
	e.preventDefault();
	e.stopPropagation();
	var btn_state = "";
	$('[class*="btn-state_"]').each(function(){
		if ($(this).hasClass('this_btn')) {
			btn_state = $(this).attr('data-filter');
		}
	})
	var check="btn-cate_";
	var cls = $(this).attr('class').split(' ');
	console.log(cls);
	$('[class*="btn-cate_"]').removeClass('this_btn');
	for (var i = 0; i < cls.length; i++) {
		if (cls[i].indexOf(check) > -1) {
			var id_emet = cls[i].slice(check.length, cls[i].length);
		}
	...
	$(this).addClass('this_btn');
	var filterValue = $(this).attr('data-filter');
	$grid.isotope({ filter: filterValue + btn_state });
})
...
$(document).on('click', '.comments-shared', function(){
	...
	$(this).next().find('textarea.commentaires').get(0).focus();
	$.ajax({
		url: '../formulaire.php',
		type: 'POST',
		context: this,
...
$(document).on('click', '.accepter_remontees', function(){
	var id_remontees = $(this).parents('.video').find('.id_remontees').val();
...
$(document).on('click', '.refuser_remontees', function(){
	var id_remontees = $(this).parents('.video').find('.id_remontees').val();
	var kats =  $('input.kats').val();
	$.ajax({
		url: '../../formulaire.php',
...
$(document).on('click', '.traitement_remontees', function(){
	var id_remontees = $(this).parents('.video').find('.id_remontees').val();
	var kats =  $('input.kats').val();
	$.ajax({
		url: '../../formulaire.php',
